...
Pass `.` when you're adding a service to a repo that already exists. It writes the same files into the current directory and takes the project name from the directory name. If any of those files are already there it refuses and writes nothing — it will not overwrite your work.
