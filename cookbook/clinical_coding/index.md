...
Set up a Medplum account and obtain client credentials. See the [FHIR Sandbox Setup Guide](https://healthchainai.github.io/HealthChain/cookbook/setup_fhir_sandboxes/#medplum) for detailed instructions.
...
First we'll need to convert the incoming CDA XML to FHIR. The [CdaAdapter](https://healthchainai.github.io/HealthChain/reference/io/adapters/cdaadapter/index.md) enables round-trip conversion between CDA and FHIR using the [InteropEngine](https://healthchainai.github.io/HealthChain/reference/interop/engine/index.md) for seamless legacy-to-modern data integration.
...
Next we'll build the medical coding pipeline: run your NLP model of choice directly in a [Pipeline](https://healthchainai.github.io/HealthChain/reference/pipeline/pipeline/index.md) node, link the extracted entities (e.g., "chronic kidney disease", "asthma") to standard clinical codes (e.g. SNOMED CT, ICD-10), and write them to the FHIR problem list with [`update_problem_list`](https://healthchainai.github.io/HealthChain/reference/io/containers/document/index.md).
...
The dictionary is pipeline-time entity linking: a deterministic mention → code mapping, standing in for a real linker (e.g. scispacy's `UmlsEntityLinker`). Its agentic counterpart is the [terminology module](https://healthchainai.github.io/HealthChain/reference/utilities/terminology/index.md) — the same lookup job exposed as a typed tool behind the `TerminologyLookup` protocol, so an LLM uses codes returned by a lookup instead of generating them.

## Set up FHIR Gateway

[FHIR Gateways](https://healthchainai.github.io/HealthChain/reference/gateway/fhir_gateway/index.md) enable your app to connect to one or more external FHIR servers (like EHRs, registries, billing systems).
...
Now let's set up the handler for [NoteReaderService](https://healthchainai.github.io/HealthChain/reference/gateway/soap_cda/index.md) method `ProcessDocument`, which will be called by Epic NoteReader when it is triggered in the CDI workflow. This is where we will combine all our components: adapter, pipeline, and writing to our configured FHIR endpoint:
...
Time to put it all together! Using [HealthChainAPI](https://healthchainai.github.io/HealthChain/reference/gateway/api/index.md), we can create a service with *both* the FHIR and NoteReader endpoints:
...
HealthChain provides a [sandbox client utility](https://healthchainai.github.io/HealthChain/reference/utilities/sandbox/index.md) which simulates the NoteReader workflow end-to-end. It loads your sample CDA document, sends it to your service via the configured endpoint, and saves the request/response exchange in an `output/` directory. This lets you test the complete integration locally before connecting to Epic.
...
1. **Sandbox runs** – Your test CDA document is:

   - Wrapped in a SOAP envelope
   - Sent to `/notereader/ProcessDocument`

1. **Artifacts** – Sandbox saves request/response XML files to the `output/` directory for inspection
...
1. Search and navigate to **Condition** resources in the search bar under the Medplum logo. You should see a list of all the Condition data you have in Medplum's FHIR server:
...
- **Build on it**: Use the extracted conditions in the [Data Aggregation example](https://healthchainai.github.io/HealthChain/cookbook/multi_ehr_aggregation/index.md) to combine with other FHIR sources.
