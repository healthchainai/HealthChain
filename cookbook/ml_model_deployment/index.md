...
Producing FHIR is the point because the resources can land in the EHR. Configure a [FHIRGateway](https://healthchainai.github.io/HealthChain/reference/gateway/fhir_gateway/index.md) source and write each assessment back, where it becomes available to dashboards, reports, and downstream workflows:
...
**Prerequisites:** a FHIR server with patient data. This example uses [Medplum](https://www.medplum.com/) — see the [FHIR Sandbox Setup guide](https://healthchainai.github.io/HealthChain/cookbook/setup_fhir_sandboxes/#medplum) for credentials, then add them to `.env`:
...
- **Real-time alerts**: To surface a score at the point of care instead of persisting it, return it as a CDS Hooks card — see the [CDS Hooks reference](https://healthchainai.github.io/HealthChain/reference/gateway/cdshooks/index.md).
- **Add more FHIR sources**: The gateway supports multiple sources — see the [FHIR Sandbox Setup guide](https://healthchainai.github.io/HealthChain/cookbook/setup_fhir_sandboxes/index.md).
