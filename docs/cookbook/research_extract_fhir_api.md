...
Each row becomes three typed FHIR resources using the [FHIR helpers](../reference/utilities/fhir_helpers.md), and every resource is validated with [`validate_resource`](../reference/utilities/fhir_helpers.md) before it enters the bundle:
...
This is the payoff: your extract becomes a queryable FHIR API. Configure a source ([free Medplum sandbox setup →](./setup_fhir_sandboxes.md#medplum)) and the [FHIRGateway](../reference/gateway/fhir_gateway.md) writes each resource:
...
!!! tip "Next Steps"
    ...
    - **Serve it to agents**: Point the [FHIRToolkit](../reference/utilities/tools.md) at your bundle or server to give LLM agents validated FHIR tools over the same data.
    - **Aggregate across sites**: Combine with the [Multi-Source Data Aggregation](./multi_ehr_aggregation.md) pattern once more than one FHIR source is involved.
