...
The [**HealthChainAPI**](https://healthchainai.github.io/HealthChain/reference/gateway/api/index.md) provides a unified interface for connecting your AI application and models to multiple healthcare systems through a single API. It automatically handles [FHIR API](https://www.hl7.org/fhir/http.html), [CDS Hooks](https://cds-hooks.org/), and [SOAP/CDA protocols](https://www.hl7.org/implement/standards/product_brief.cfm?product_id=7) with [OAuth2 authentication](https://oauth.net/2/).

[(Full Documentation on Gateway)](https://healthchainai.github.io/HealthChain/reference/gateway/gateway/index.md)
...
The [**Terminology**](https://healthchainai.github.io/HealthChain/reference/utilities/terminology/index.md) seam adds code lookup (`lookup_code("metoprolol")`) with a bundled demo catalog that swaps for your terminology server in production.

## Agent Tools

[**FHIRToolkit**](https://healthchainai.github.io/HealthChain/reference/utilities/tools/index.md) packages the build/validate/read/lookup surface as typed agent tools — flat schemas a model can fill reliably, with errors returned as values an agent loop can read and correct.

[(Full Documentation on Agent Tools)](https://healthchainai.github.io/HealthChain/reference/utilities/tools/index.md)
...
The [**SandboxClient**](https://healthchainai.github.io/HealthChain/reference/utilities/sandbox/index.md) plays the role of the EHR: it fires realistic, protocol-shaped requests (CDS Hooks, SOAP/CDA) at your service and lets you inspect what comes back — a closed verify loop that needs no real EHR, whether you wrote the integration yourself or an agent built it for you. Load test datasets, send requests, and validate responses in a few lines of code.

[(Full Documentation on Sandbox)](https://healthchainai.github.io/HealthChain/reference/utilities/sandbox/index.md)

### Workflows

A [**workflow**](https://healthchainai.github.io/HealthChain/reference/utilities/sandbox/#workflow-protocol-compatibility) represents a specific event in an EHR system that triggers your service (e.g., `patient-view` when opening a patient chart, `encounter-discharge` when discharging a patient).
...
[**Dataset Loaders**](https://healthchainai.github.io/HealthChain/reference/utilities/sandbox/#dataset-registry) are shortcuts for loading common clinical test datasets from file. Currently available:
...
[**Pipelines**](https://healthchainai.github.io/HealthChain/reference/pipeline/pipeline/index.md) are the supporting primitive behind protocol flows: they compose processing steps around your own models, and gateway handlers (CDS Hooks, NoteReader) pass each request through them as a [**Document**](https://healthchainai.github.io/HealthChain/reference/io/containers/containers/index.md).
...
[(Full Documentation on Containers)](https://healthchainai.github.io/HealthChain/reference/io/containers/containers/index.md)
...
[**Components**](https://healthchainai.github.io/HealthChain/reference/pipeline/components/components/index.md) are reusable, stateful classes that encapsulate specific processing logic for your pipeline, and you can easily implement your own.
...
[(Full Documentation on Components)](https://healthchainai.github.io/HealthChain/reference/pipeline/components/components/index.md)
...
You can process legacy healthcare data formats too. [**Adapters**](https://healthchainai.github.io/HealthChain/reference/io/adapters/adapters/index.md) convert between healthcare formats like [CDA](https://www.hl7.org/implement/standards/product_brief.cfm?product_id=7) and your pipeline — just parse, process, and format without worrying about low-level data conversion.

[(Full Documentation on Adapters)](https://healthchainai.github.io/HealthChain/reference/io/adapters/adapters/index.md)
...
For complete, runnable examples of common clinical tasks — medical coding, discharge summarization, ML risk scoring — start from a [**cookbook recipe**](https://healthchainai.github.io/HealthChain/cookbook/clinical_coding/index.md) and adapt it. Each recipe builds its pipeline from `Pipeline` + `add_node` directly, so every step is visible and easy to change.

[(Full Documentation on Pipelines)](https://healthchainai.github.io/HealthChain/reference/pipeline/pipeline/index.md)
...
[(Full Documentation on Interoperability Engine)](https://healthchainai.github.io/HealthChain/reference/interop/interop/index.md)
