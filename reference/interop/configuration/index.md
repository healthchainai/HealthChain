...
CDA configurations primarily concern the extraction and rendering processes on the ***document*** and ***section*** level. While [Templates](https://healthchainai.github.io/HealthChain/reference/interop/templates/index.md) are used to define the syntactic structure of the document, the YAML configurations are used to define the semantic content of the document structure, such as template IDs, status codes, and other metadata.
...
(Full Documentation on [Mappings](https://healthchainai.github.io/HealthChain/reference/interop/mappings/index.md))
...
1. **Environment configuration** (environments/{env}.yaml) - overrides defaults
1. **Runtime overrides** (via API) - highest precedence
