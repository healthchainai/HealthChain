...
Parsers are responsible for extracting structured data from healthcare document formats. The module includes a built-in parser for CDA documents.
...
Internally, it uses [xmltodict](https://github.com/martinblech/xmltodict) to parse the XML into a dictionary, validates the dictionary with [Pydantic](https://docs.pydantic.dev/), and then maps each entry to the section keys. See [Working with xmltodict in HealthChain](https://healthchainai.github.io/HealthChain/reference/interop/xmltodict/index.md) for more details.
...
If the CDA document contains a section that has no matching entry in the configured sections (e.g. an allergies section, since the bundled configs only cover problems, medications, and notes), the parser logs a warning naming the section's title and code and skips it.

([Full Documentation on Configuration](https://healthchainai.github.io/HealthChain/reference/interop/configuration/index.md))
